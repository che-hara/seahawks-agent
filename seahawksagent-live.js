...
function dashboardFragments() {
  ...
  const pendingHtml = state.pendingPost
    ? `<div class="pending-post">
        ...
        <textarea id="post-edit" class="post-edit" maxlength="300" data-original="${escapeHtml(state.pendingPost.text)}" oninput="updateCharCount(this)">${escapeHtml(state.pendingPost.text)}</textarea>
...
function renderDashboard() {
  const f = dashboardFragments();
  const {
    phaseLabel, phaseClass, scoreHtml, pendingHtml,
    recentHtml, sentimentHtml, vibeHtml, counterHtml,
  } = f;
...
<body>
  <header>
    ...
    <span class="status-pill ${phaseClass}" id="status-pill"><span class="dot"></span>${phaseLabel}</span>
  ...
  <main>
    <div class="card">
      ...
      <div id="panel-score">${scoreHtml}</div>
    </div>
    <div class="card">
      <div class="card-title">Game Vibe</div>
      <div id="panel-vibe">${vibeHtml}</div>
    </div>
    <div class="card full-width">
      <div class="card-title">Pending Post</div>
      <div id="panel-pending">${pendingHtml}</div>
      <div class="posts-counter" id="posts-counter">${counterHtml}</div>
    </div>
    <div class="card">
      <div class="card-title">Recent Posts</div>
      <div id="panel-recent">${recentHtml}</div>
    </div>
    <div class="card">
      <div class="card-title">What Fans Are Saying</div>
      <div id="panel-sentiment">${sentimentHtml}</div>
    </div>
  </main>
  ...
  <script>
    ...
    function schedule(phase) {
      ...
      const base = REFRESH_MS[phase] || DEFAULT_REFRESH_MS;
      refreshTimer = setTimeout(refresh, backoff ? Math.min(base * backoff, 60000) : base);
    }

    async function refresh() {
      let f;
      try {
        const r = await apiFetch("/api/fragments");
        if (!r.ok) { backoff = Math.min(backoff + 1, 4); return schedule("waiting"); }
        f = await r.json();
        backoff = 0;
      } catch (e) {
      ...
      if (ts) {
        if (f.lastUpdated) ts.setAttribute("data-ts", f.lastUpdated);
        else { ts.removeAttribute("data-ts"); ts.textContent = "--"; }
      }

      const pill = document.getElementById("status-pill");
      if (pill) {
        pill.className = "status-pill " + f.phaseClass;
        pill.innerHTML = '<span class="dot"></span>' + f.phaseLabel;
      }
    ...
    document.addEventListener("visibilitychange", () => {
      if (!document.hidden) { clearTimeout(refreshTimer); refresh(); }
    });
...
function serveDashboard() {
  const server = http.createServer((req, res) => {
    ...
    if (pathname === "/api/fragments" && method === "GET") {
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify(dashboardFragments()));
      return;
    }
